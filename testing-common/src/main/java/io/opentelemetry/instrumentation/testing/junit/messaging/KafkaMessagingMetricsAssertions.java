...
public final class KafkaMessagingMetricsAssertions {
  ...
  public static void assertProcessMetricPointCounts(
      InstrumentationExtension testing,
      String instrumentationName,
      int durationPointCount,
      Integer consumedPointCount) {
    if (!emitStableMessagingSemconv()) {
      return;
    }
    ...
    if (consumedPointCount != null) {
      testing.waitAndAssertMetrics(
          ...
          metrics ->
              metrics.satisfiesExactly(
                  metric ->
                      assertThat(metric.getLongSumData().getPoints()).hasSize(consumedPointCount)));
    }
  }
